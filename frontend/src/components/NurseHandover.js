...
const NurseHandover = ({ patientId }) => {
  ...
  const fetchHandovers = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/patients/${patientId}/nurse-handovers`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to submit nurse handover');
      }
    } catch (error) {
      console.error('Error submitting nurse handover:', error);
    }
  };

  return (
    <div className="nurse-handover-container">
      <h3>Nurse Handovers</h3>
      ...
      <div className="handovers-list">
        ...
        {handovers.length > 0 ? (
          ...
          <p>No nurse handovers recorded.</p>
        )}
      </div>
    </div>
  );
